...
pub fn toggle_mini_window(app: AppHandle, to_mini: bool) {
    if let Some(main) = app.get_webview_window("main") {
        ...
        let new_logical_size = if to_mini {
            logical_mini_size
        } else {
            logical_main_size
        };
        let cur_logical_size = if to_mini {
            logical_main_size
        } else {
            logical_mini_size
        };
        ...
        if let Ok(Some(monitor)) = main.current_monitor() {
            ...
            let dist_right =
                (monitor_pos.x + monitor_size.width as i32) - (cur_pos.x + cur_size.width as i32);
            let dist_bottom =
                (monitor_pos.y + monitor_size.height as i32) - (cur_pos.y + cur_size.height as i32);
            ...
            if !to_mini {
                ...
                if cur_pos.x + target_outer_width
                    > monitor_pos.x + monitor_size.width as i32 + right_border
                {
                    cur_pos.x = monitor_pos.x + monitor_size.width as i32 + right_border
                        - target_outer_width;
                }
                if cur_pos.y + target_outer_height
                    > monitor_pos.y + monitor_size.height as i32 + border_y
                {
                    cur_pos.y =
                        monitor_pos.y + monitor_size.height as i32 + border_y - target_outer_height;
                }
