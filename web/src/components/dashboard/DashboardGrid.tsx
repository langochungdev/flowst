  ...
  const grid = useMemo(() => {
    ...
    return Array.from({ length: days }, (_, rowIndex) =>
      Array.from({ length: weeks }, (_, colIndex) => {
        ...
        const isWrongYear =
          effectiveYear !== "last1year" && date.getFullYear() !== Number(effectiveYear);

        if (isFuture || isWrongYear) {
          level = -1;
        } else if (
          date.getFullYear() === currentYear &&
          date.getMonth() === getMockedDate().getMonth() &&
          date.getDate() === getMockedDate().getDate()
        ) {
                    ...
                    {isNewMonthLeft && (
                      <div
                        style={{
                          position: "absolute",
                          left: "-1px",
                          top: rowIndex > 0 && !isNewMonthTop ? "-1px" : "0",
                          bottom: 0,
                          width: "1px",
                          backgroundColor: "#555",
                          opacity: 0.5,
                    ...
                    {isNewMonthTop && (
                      <div
                        style={{
                          position: "absolute",
                          top: "-1px",
                          left: colIndex > 0 && !isNewMonthLeft ? "-1px" : "0",
                          right: 0,
                          height: "1px",
                          backgroundColor: "#555",
                          opacity: 0.5,
