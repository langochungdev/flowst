...
export default function DashboardStats({ selectedCategories, timeFilter, customDays }: { selectedCategories: string[], timeFilter: string, customDays: number | string }) {
  ...
  }, [timeFilter, customDays, history, todayTotalTime, todayCategoryBreakdown, categories, selectedCategories]);

  return (
    <div style={{ display: "flex", flexDirection: "column", background: "var(--bg-glass)", padding: "16px", border: "1px solid var(--glass-border)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
        <h3 style={{ margin: 0, fontSize: "18px" }}>Total Stats</h3>
        <div style={{ display: "flex", gap: "6px", alignItems: "center" }}>
            <span style={{ fontSize: "12px", color: "var(--text-muted)", fontWeight: "bold", background: "#111", padding: "2px 8px", borderRadius: "12px", border: "1px solid #222" }}>{stats.days} days</span>
            <span style={{ fontSize: "12px", color: "var(--text-muted)", fontWeight: "bold", background: "#222", padding: "2px 8px", borderRadius: "12px" }}>{stats.dateRange}</span>
        </div>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "6px" }}>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginBottom: "12px" }}>
          ...
          <div style={{ display: "flex", marginTop: "4px" }}>
              <span style={{ fontSize: "14px", color: "#00F2F6", fontWeight: "bold" }}>~{stats.avgHours}h / day</span>
          </div>
        </div>
          ...
          <div style={{ width: "100%", display: "flex", flexDirection: "column", gap: "6px" }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "8px 12px", justifyContent: "center" }}>
              {stats.breakdownList.map(b => (
                <div key={b.name} style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "14px", background: "#111", padding: "4px 8px", borderRadius: "6px", border: "1px solid #222" }}>
                  <span style={{ color: b.color, fontWeight: "bold" }}>{b.name}:</span>
                  <span style={{ fontWeight: "bold" }}>{b.hours}h</span>
                  <span style={{ fontSize: "12px", color: "#888", marginLeft: "4px" }}>(~{b.avgHours}h/d)</span>
                </div>
              ))}
            </div>
          </div>
