...
export default function DashboardWindow() {
            ...
            switch (timeFilter) {
              ...
              default:
                if (!isNaN(Number(timeFilter))) {
                  ...
                  numDays = Math.ceil((endOfYear.getTime() - startOfYear.getTime()) / (1000 * 3600 * 24)) + 1;
                }
                break;
            }
    ...
    return (
        ...
        >
            {!isExporting && (
                <div style={{ position: "absolute", top: 12, left: 12, zIndex: 20, display: "flex", gap: "8px" }}>
                    <button
                    ...
                    <div className="no-drag" style={{ position: "relative" }} ref={categoryDropdownRef}>
                        ...
                        {isFilterOpen && (
                            <div style={{
                                    ...
                                    minWidth: "150px", zIndex: 100, boxShadow: "0 4px 12px rgba(0,0,0,0.5)"
                                }}>
                                <div style={{ fontSize: "12px", color: "#888", marginBottom: "4px", paddingBottom: "4px", borderBottom: "1px solid #444" }}>Categories</div>
                                {categories.map(cat => (
                                     <label key={cat.id} style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "12px", cursor: "pointer" }}>
                                         <input 
                                             type="checkbox" 
                    ...
                    <div className="no-drag" style={{ position: "relative" }} ref={timeDropdownRef}>
                        ...
                        >
                            <Calendar size={14} /> {timeOptions.find(o => o.value === timeFilter)?.label || "Time"}
                        </button>
                        {isTimeFilterOpen && (
                            <div style={{
                                    position: "absolute", top: "100%", left: 0, marginTop: "4px",
                                    background: "#222", border: "1px solid #444", borderRadius: "4px",
                                    display: "flex", flexDirection: "column", padding: "4px", gap: "2px",
                                    minWidth: "130px", zIndex: 100, boxShadow: "0 4px 12px rgba(0,0,0,0.5)"
                                }}>
                                    {timeOptions.map(opt => (
                                         <div 
                                             key={opt.value} 
                                             ...
                                             style={{
                                                 padding: "6px 12px", fontSize: "12px", cursor: "pointer", borderRadius: "4px",
                                             ...
                                             onMouseEnter={(e) => { if (timeFilter !== opt.value) e.currentTarget.style.background = "#333" }}
                                             onMouseLeave={(e) => { if (timeFilter !== opt.value) e.currentTarget.style.background = "transparent" }}
                                         >
                                             {opt.label}
                                         </div>
                                    ))}
                                    {timeFilter === "custom" && (
                                        <div style={{ padding: "4px 8px", display: "flex", alignItems: "center", gap: "8px" }}>
                                            ...
                                            <input 
                                                ...
                                                style={{ width: "50px", background: "#111", border: "1px solid #444", color: "#fff", borderRadius: "4px", padding: "2px 4px", fontSize: "12px" }}
            ...
            >
                <div style={{ flex: "0 0 auto" }}>
                    <DashboardStats selectedCategories={selectedCategories} timeFilter={timeFilter} customDays={customDays} />
                </div>

                <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
                    <DashboardChart selectedCategories={selectedCategories} timeFilter={timeFilter} customDays={customDays} />
                </div>
